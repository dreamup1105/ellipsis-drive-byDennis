    ...
    return (
      <div className="application">
        <Helmet>
          ...
          <link rel="canonical" href="https://ellipsis-drive.com/solution/" />
        </Helmet>
      </div>
    )
  }
...
const Solution = ({ location, data }) => {
  let locationParts = location.pathname.split('/');
  let articleUid = locationParts[locationParts.length - 1];

  let article = data.allPrismicSolution.nodes.find((x) => x.uid === articleUid);
